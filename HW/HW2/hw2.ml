...
let rec replace_leaf_at : dTree->(int list * int) list-> dTree = fun t graph ->
	let rec replace_helper : dTree -> int list -> int -> dTree = fun t graph newVal ->
		match t,graph with
		| Leaf(_),[] -> Leaf(newVal)
		| Node(c,lt,rt),x::xs -> if(x == 0) then Node(c,replace_helper lt xs newVal,rt) 
		  else  Node(c,lt,replace_helper rt xs newVal)
...
let bf_to_dTree : (char list * (int list * int) list) -> dTree = fun pairRep ->
	let (r1,r2) = pairRep in replace_leaf_at (list_to_tree r1) r2
