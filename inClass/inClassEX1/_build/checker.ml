...
let rec contains = fun x id ->
    match x with 
    | [] -> false
    | x::xs -> let (r1,r2) = x in if r1 = id then true else false || (contains xs id)

let rec has_duplicates = fun x ->
    match x with
    | [] -> false
    | x::xs -> let (r1,r2) = x in if (contains xs r1) then true else false || has_duplicates xs
...
and
  ...
  | Debug ->
    ...
    UnitType
  | Record(fs) -> if has_duplicates fs then failwith "has duplicates" else RecordType(List.map (fun x -> let (r1,r2) = x in (r1,(type_of_expr tenv r2))) fs)
  | Proj(e,field_id) ->  let test  = type_of_expr tenv e in  (match test with 
    |RecordType(actualthing) -> List.assoc field_id actualthing
    | _ -> failwith "wtf")
