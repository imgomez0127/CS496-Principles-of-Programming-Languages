...
let rec value_of_operand env e =
  match e with
  | Var id -> from_some @@ apply_env g_store env id
  | _ -> RefVal (Store.new_ref g_store (ThunkVal(e,env)))
and value_of_thunk env = function
    | _ -> failwith "Expected a Thunk"
and apply_proc f refv =
  match f with
  | ProcVal (x,b,env) ->
...
and
  ...
  | Var id          ->
    ...
    | Some aRef -> match Store.deref g_store @@ refVal_to_int aRef with
      | ThunkVal(e,en) -> let value_of_thunk = eval_expr en e
        in Store.set_ref g_store (refVal_to_int aRef) value_of_thunk;
          value_of_thunk
      | ev -> ev )
  | ITE(e1, e2, e3)    ->
  ...
  | App(e1,e2)     ->
    let v1 = eval_expr en e1 in
    let v2 = value_of_operand en e2 in
    apply_proc v1 v2
...
and
  eval_prog  (Ast.AProg e) =
     eval_expr init_env e
