...
let rec find = fun lst id ->
    match lst with
    | [] -> failwith "not found";
    | x::xs -> let (r1,r2) = x in if (r1 = id) then r2 else find xs id 

let rec apply_proc f a = match f with
  | ProcVal (x,b,env) ->
...
and
  ...
  | Add(e1, e2)    -> let v1 = eval_expr en e1 in
...
(*second answer to record*)
(*let rec toRecordVal = fun lst ->
                  match lst with 
